...
#define ACORAL_EVENT_MSGCTR ACORAL_EVENT_MIN_NUM+2 ///<消息队列
...
typedef struct {
	acoral_res_t         res; ///<event也是一种资源
  	acoral_u8            type; ///<类型，ACORAL_EVENT_SEM（信号量）或ACORAL_EVENT_MUTEX（互斥量）或ACORAL_EVENT_MSGCTR（消息队列）
	acoral_32           count; ///<当type是互斥量时：\
								23~16位表示这个互斥量的优先级天花板。\这个值在互斥量被创建的时候就确定了且不会改变。\
								15~8位表示这个互斥量已经被占用时，因为尝试申请互斥量而被阻塞的线程中最高的优先级。\
								7~0位在互斥量没有被上锁时，为全1，表示互斥量可用；在被上锁，也就是被占用时，会被赋值为占用它的线程的原始优先级。之所以说是原始优先级，是因为占用线程在使用互斥量的过程中可能被提升优先级，那在释放互斥量之后就要恢复之前的优先级，那就是从count成员的7~0位取值。\
								当type是信号量或消息队列时，自行探索。
