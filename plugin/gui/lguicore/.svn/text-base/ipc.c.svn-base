	...
	case MTYPE_IPC:
		...
		case LMSG_IPC_CHAR:
			if(!_lGUI_pFocus)
				...
				SendMessage(_lGUI_pFocus,LMSG_CHAR,pAttachData,(LPARAM)NULL);
			}	
			break;
...
)
{
	...
	if(!_lGUI_pAppStat)
		return;
	SendMsgByServer(_lGUI_pAppStat,LMSG_IPC_CHAR,(void*)ch,0);
}
...
)
{
	if(!_lGUI_pAppStat)
		return;
	SendMsgByServer(_lGUI_pAppStat,LMSG_IPC_KEYDOWN,(void*)byteKeyValue,0);
}
